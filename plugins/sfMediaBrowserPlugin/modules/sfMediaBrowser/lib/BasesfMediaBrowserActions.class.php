...
class BasesfMediaBrowserActions extends sfActions
{
  ...
  {
    $oldFile = new sfMediaBrowserFileObject($request->getParameter('file'));
    $newDir = new sfMediaBrowserFileObject($request->getParameter('dir'), $this->root_path);
    $this->checkPath($oldFile->getPath());
    $this->checkPath($newDir->getPath());
    
    $error = null;
    $moved = @rename($oldFile->getPath(), $newDir->getPath().'/'.$oldFile->getName(true));
    if(!$moved)
    {
      $this->logMessage(sprintf('Failed renaming "%s" to "%s".', $oldFile->getPath(), $newDir->getPath()), 'err');
    }
    ...
    if($request->isXmlHttpRequest())
    {
      ...
      elseif(file_exists($newDir->getPath().'/'.$oldFile->getName(true)))
      {
      ...
      else
      {
        ...
        $response = array('status' => 'error', 'message' => $oldFile->getPath());
      }
